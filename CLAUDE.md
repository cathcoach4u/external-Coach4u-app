...
## Latest
- **v0.5.110** — Added the **Coach role**. Coaches are admin-equivalent for data and team operations (full edit access, can invite/remove other members) but are intended to be exempt from per-seat billing when billing is wired. Use case: the business coach lives inside the client's account, edits worksheets / scorecard / goals / sessions, without consuming one of the client's 3 included seats. Coaches cannot rename or delete the business itself — that stays admin-only. **Requires SQL** — see `supabase/v0.5.110-delta.sql`. Schema: relaxed `team_members.role` CHECK to allow `'coach'`; updated `user_admin_org_ids()` helper to include coaches → every existing RLS policy that gates writes via this helper automatically grants coaches admin-write access (no per-table edits needed); updated `invite_team_member()` and `remove_team_member()` so coaches can manage the team. UI: invite modal gets a third "Coach — admin rights, not billed as a user" option with help text; new purple `.role-coach` pill style in the team list; "manage team" UI now activates for coaches as well as admins.

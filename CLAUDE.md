...
## Latest
- **v0.5.115** — Account-level companion to v0.5.114. New **`account-ops-plans.html`** — a carousel that steps through every business in the active account, rendering each business's one-page operations doc (quarterly goals · weekly numbers · open issues). Linked from `account-operations.html` via a "View One-Page Operations — All Businesses" button, mirroring how `account-strategy.html` links to `account-plans.html`. Same prev/next/dot/keyboard nav as the strategy carousel. Multi-tenant-aware: the membership query is scoped by `organisations.subscription_id = activeSub.id`, so a user who owns multiple client accounts only sees the businesses inside the currently-selected account. Also added the same scoping (+ loaded `js/active-org.js`) to **`account-plans.html`**, which had the same pre-existing leak: before this fix, that strategy carousel showed businesses across every account the user owned, mixed together.
- **v0.5.114** — New **One-Page Operations** doc (`one-page-operations.html`), parallel to the existing `one-page-plan.html`. Landscape A4, three columns: Quarterly Goals (rocks for the current quarter, with status icon + owner + Company-Goal flag), Weekly Numbers (scorecard_metrics × last 6 weeks, cells coloured teal/red against the metric's goal), Open Issues (numbered list with owner). Reads from the same Supabase tables as `goals.html` / `scorecard.html` / `issues.html`, scoped by `active organisation_id`. Print button → A4 landscape PDF, same `@page` rules and print stylesheet pattern as the strategy doc. Linked from `operations.html` via the same `view-plan-btn` style.
- **v0.5.113** — Fixed cross-account data leak on `business.html`. Symptom: opening the business dashboard for a new client account showed Coach4U sample text ("Sign 20 new clients..." 1-year goal + Integrity / Growth Mindset / Client First / Accountability values pills) even though no `targets` / `core_values` row existed in Supabase for that org. Root cause: those panels had hardcoded HTML defaults that the JS only overwrote IF Supabase returned a row — when the row was missing for a new biz, the Coach4U sample stayed visible. The HTML defaults are now empty-state placeholders ("No 1-year goal set yet." / "No core values set yet.") and the JS toggles them explicitly. Also dropped the 5 dead `coach4u_*` localStorage seeds from `ensureSeeds()` — every strategy worksheet now reads/writes Supabase, so those keys had no readers and just polluted localStorage globally across accounts.

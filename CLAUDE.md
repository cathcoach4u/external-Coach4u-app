...
## Latest
- **v0.5.92** — `account-plans.html` switched from "all plans stacked" to a **carousel**: one business' one-pager at a time, with **prev/next arrows + dot indicators + keyboard arrow keys** to step through. The arrow buttons preview the next/previous business name. Centre shows the current business name + "Business X of Y" counter + clickable dots for direct jump. Stacking all businesses on one page was overwhelming and didn't match the mental model — the carousel pattern matches "I want to look at one, then the next" naturally. Print now prints only the current view.
- **v0.5.91** — First cross-business topic page: **`account-plans.html`** (One-Page Plans across all businesses). Reads the caller's active `team_members`, pulls `core_values` / `core_focus` / `targets` / `marketing_strategy` / `leadership_team_members` per org in 5 parallel queries.

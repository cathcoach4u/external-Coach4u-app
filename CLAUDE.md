...
## Latest
- **v0.5.82** — Made the "Manage businesses ›" link on the dashboard always visible (was previously only shown when the user had 2+ businesses, so single-business users had no path to `my-businesses.html` to add their second). Label adapts: "Switch business ›" when 2+, "Manage businesses ›" when 1.
- **v0.5.81** — "My Businesses" page + active-org tracking. New `my-businesses.html` lists every org the user is a member of with role pill (admin / member), highlights the current active one, and has a "+ Create New Business" modal that calls `bootstrap_organisation` again (same function as setup — handles both first business and additional). New `js/active-org.js` exposes `window.activeOrg.get/set/clear` backed by localStorage key `coach4u_active_org_id`. `index.html` now: resolves the user's team_member rows, picks the active org (stored selection if still valid, else first), shows the business name at the top of the dashboard, and shows a "Switch business ›" link when the user has more than one org. `setup.html` sets the new org as active immediately after creation. **Still localStorage-backed for data tools** — wiring tools to Supabase queries starts v0.5.83.

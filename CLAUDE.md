...
## Latest
- **v0.5.91** — First cross-business topic page: **`account-plans.html`** (One-Page Plans across all businesses). Reads the caller's active `team_members`, pulls `core_values` / `core_focus` / `targets` / `marketing_strategy` / `leadership_team_members` per org in 5 parallel queries, renders a compact one-pager card per business (same 3-column layout as `one-page-plan.html`) stacked vertically. Print-friendly (landscape A4, page-break per business). Empty fields render as muted italic placeholders. If every business returns empty plan data, a yellow banner explains the data-layer migration is pending. Topic launcher card flipped from coming-soon to available.
